...
parse_dh_params_impl(Data) ->
    %% Skip version sub-packet (8 bytes)
    <<_PktLen1:16, _PktType1:16, _Version:32, Rest1/binary>> = Data,

    %% Read algorithm ID sub-packet (5 bytes)
    <<_PktLen2:16, _PktType2:16, IntegrityAlgoId:8, Rest2/binary>> = Rest1,

    %% Read DH gen length sub-packet (6 bytes, length in bits)
    <<_PktLen3:16, _PktType3:16, DHGenLen:16, Rest3/binary>> = Rest2,

    %% Read DH prime length sub-packet (6 bytes)
    <<_PktLen4:16, _PktType4:16, _DHPrimeLen:16, Rest4/binary>> = Rest3,

    %% Calculate expected byte length: (bitLen + 7) / 8
    ByteLen = (DHGenLen + 7) div 8,

    %% Read generator bytes sub-packet
    <<GenPktLen:16, _GenPktType:16, Rest5/binary>> = Rest4,
    GenBytes = GenPktLen,
    <<Gen:GenBytes/binary, Rest6/binary>> = Rest5,

    %% Read prime bytes sub-packet
    <<PrimePktLen:16, _PrimePktType:16, Rest7/binary>> = Rest6,
    PrimeBytes = PrimePktLen,
    <<Prime:PrimeBytes/binary, Rest8/binary>> = Rest7,

    %% Read server public key sub-packet
    <<ServerPubKeyPktLen:16, _ServerPubKeyPktType:16, Rest9/binary>> = Rest8,
    ServerPubKeyBytes = ServerPubKeyPktLen,
    <<ServerPubKey:ServerPubKeyBytes/binary, Rest10/binary>> = Rest9,
    ...
    case byte_size(ServerPubKey) =:= ByteLen andalso byte_size(Prime) =:= ByteLen of
        true ->
            %% Read IV sub-packet
            <<IVPktLen:16, _IVPktType:16, Rest11/binary>> = Rest10,
            IVBytes = IVPktLen,
            <<IV:IVBytes/binary, RestData/binary>> = Rest11,
